\begin{table}
\caption{Distance sensitivity on the same 356-star final-strict sample; the two columns differ only in the distance estimator.}
\label{tab:sensitivity}
\begin{tabular}{lll}
\toprule
Quantity & Inverse parallax & Bailer--Jones \\
\midrule
Sample size & 356 & 356 \\
$P_{\mathrm{unbound}} > 0.5$ & 48 & 3 \\
$P_{\mathrm{unbound}} > 0.7$ & 29 & 1 \\
$P_{\mathrm{unbound}} > 0.9$ & 12 & 1 \\
Stars downgraded across $P=0.5$ & --- & 45 \\
Stars promoted across $P=0.5$ & --- & 0 \\
Median $r_{\mathrm{med,geo}}/(1000/\varpi)$ & --- & 0.88 \\
\bottomrule
\end{tabular}
\end{table}
